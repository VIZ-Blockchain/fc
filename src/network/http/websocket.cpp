...
namespace fc {
    namespace http {
        ...
        namespace detail {
                ...
                websocket_server_impl(const leaky_bucket_rules& limit)
                    ...
                    _server.set_open_handler([&](connection_hdl hdl) {
                        _server_thread.async([&]() {
                            auto new_con = std::make_shared<websocket_connection_impl<websocket_server_type::connection_ptr>>(_server.get_con_from_hdl(hdl),limit);
                    ...
                    _server.set_http_handler([&](connection_hdl hdl) {
                        _server_thread.async([&]() {
                            auto current_con = std::make_shared<websocket_connection_impl<websocket_server_type::connection_ptr>>(_server.get_con_from_hdl(hdl),limit);
            ...
            public:
                websocket_tls_server_impl(const leaky_bucket_rules& limit,const std::string &server_pem, const std::string &ssl_password) : _server_thread(fc::thread::current()) {
                    ...
                    _server.set_open_handler([&](connection_hdl hdl) {
                        _server_thread.async([&]() {
                            auto new_con = std::make_shared<websocket_connection_impl<websocket_tls_server_type::connection_ptr>>(_server.get_con_from_hdl(hdl),limit);
                            _on_connection(_connections[hdl] = new_con);
                        }).wait();
                    });
                    _server.set_message_handler([&](connection_hdl hdl, websocket_server_type::message_ptr msg) {
                    ...
                    _server.set_http_handler([&](connection_hdl hdl) {
                        _server_thread.async([&]() {
                            ...
                            auto current_con = std::make_shared<websocket_connection_impl<websocket_tls_server_type::connection_ptr>>(_server.get_con_from_hdl(hdl),limit);
                            try {
                                _on_connection(current_con);
                    ...
                    _client.set_fail_handler([=](connection_hdl hdl) {
                        ...
                        if (_connected && !_connected->ready())
                            _connected->set_exception(
                                    exception_ptr(new FC_EXCEPTION(exception, "${message}", ("message", message))));
                        if (_closed)
                            _closed->set_value();
                    });
        ...
        void websocket_server::listen(const fc::ip::endpoint &ep) {
            my->_server.listen(
                    boost::asio::ip::tcp::endpoint(boost::asio::ip::address_v4(uint32_t(ep.get_address())), ep.port()));
        }
        ...
        websocket_tls_client::websocket_tls_client(const std::string &ca_filename) : my(
                new detail::websocket_tls_client_impl(ca_filename)) {}
